...
/// Extensions [`scan_music_folder`] treats as audio, lower-case.
///
/// Public so a second front end can be **tested against it** rather than against a
/// copy of it. `eko-cli` walks the tree itself — it needs per-file progress that
/// `scan_music_folder` cannot report — so it keeps its own list, and that list
/// asserts equality with this one. While this was private the CLI's guard was
/// tautological: it could only compare against a literal duplicated inside its own
/// assertion, so a format added here went unnoticed.
