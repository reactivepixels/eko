...
//!
//! This module is EKO's single source of audio truth, and it has no UI dependency.
//! The frontend (TypeScript/React) calls the Tauri commands in
//! `eko-tauri`'s `commands::engine`, which are thin wrappers over the [`Engine`]
//! methods defined here; everything else — hardware negotiation, decoding, DSP, and
//! spectrum analysis — lives in Rust. No audio data crosses the IPC boundary.
//!
...
// The biquad filter lives in `crate::biquad` (free DSP) so the
...
/// Snapshot of engine state returned by [`Engine::status`].
...
/// A `reqwest` fetch error rendered safely for logging.
///
/// **`without_url` is the load-bearing call here, not a tidy-up.** The URL behind
/// [`Source::Url`] is the pre-signed Subsonic `stream` URL: it carries `u` (username),
/// `t` (the md5 of password + salt) and `s` (the salt). Under Subsonic's auth scheme that
/// triple is a **replayable credential** — anyone holding it can issue requests as the
/// user until the password changes. `reqwest`'s `Display` appends `" for url ({url})"` to
/// almost every error it produces, so a bare `{e}` here writes a live credential to
/// stderr, where it can reach a terminal scrollback, a log file, or a shared CI record.
///
/// This is the same hazard `eko_net::client::safe_message` exists to strip on the
/// Subsonic API path, and `pro::offline::safe_reqwest` on the download path — fixed the
/// same way. No scrubber is needed on top: this site never wraps the error in an
/// `io::Error`, so `without_url` reaches it directly.
///
/// Cost to diagnosis: none. The error's kind — `"error sending request"`,
/// `"error decoding response body"` — is what identifies the fault, and the server being
/// streamed from is already known to whoever started playback.
fn safe_fetch_message(e: reqwest::Error) -> String {
            ...
            let done = Arc::new(AtomicBool::new(false));
            {
                ...
                std::thread::spawn(move || {
                    ...
                    match client.get(&url).send() {
                        ...
                        Err(e) => eprintln!("engine fetch: {}", safe_fetch_message(e)),
                    }
...
// ── Public API ────────────────────────────────────────────────────────────────
//
// These were `#[tauri::command]` functions before the engine moved into `eko-core`.
// The bodies are unchanged; each `engine: tauri::State<Engine>` parameter became
// `&self`, and every `engine.` became `self.`. `crates/eko-tauri/src/commands/engine.rs`
...
impl Engine {
    ...
    pub fn set_device(&self, name: Option<String>) {
        *self.device_pref.lock().unwrap() = name.filter(|s| !s.is_empty());
    }
    ...
    ///
    /// The Pro *entitlement* check is NOT here: it needs a `tauri::AppHandle` to locate
    /// the licence file, so it stays in `eko-tauri`'s `engine_set_param_eq` wrapper and
    /// still runs before this method is ever called.
    #[cfg(feature = "pro")]
    pub fn set_param_eq(
        &self,
        enabled: bool,
    ...
    pub fn set_replaygain(&self, gain_db: Option<f32>) {
        if let Some(sh) = self.shared.0.lock().unwrap().as_ref() {
            sh.rg_gain
...
#[cfg(feature = "pro")]
pub fn eq_curve(bands: Vec<crate::pro::param_eq::ParamBand>, preamp: f64) -> Vec<f32> {
...
///
/// As with [`Engine::set_param_eq`], the Pro entitlement check stays in the
/// `eko-tauri` wrapper — it needs a `tauri::AppHandle` to find the licence file.
#[cfg(feature = "pro")]
pub fn parse_autoeq(text: String) -> Result<serde_json::Value, String> {
...
/// `eko-tauri` wrapper — it needs a `tauri::AppHandle` to find the licence file.
#[cfg(feature = "pro")]
pub fn import_autoeq_file(path: String) -> Result<serde_json::Value, String> {
...
mod tests {
    ...
    /// A signed Subsonic `stream` URL of exactly the shape [`Source::Url`] is handed:
    /// `t` is an md5 (32 hex chars) that, with `u` and `s`, replays as the user.
    const SIGNED_STREAM_URL: &str = "https://music.example.com/rest/stream.view\
         ?id=tr-1&u=rod&t=5f4dcc3b5aa765d61d8327deb882cf99&s=abc123&v=1.16.1&c=eko";

    /// The stderr write at the `Source::Url` fetch site must never carry the signed URL.
    ///
    /// This matters twice over: the token is a replayable credential, and `eko-cli`'s TUI
    /// never redirects fd 2 — so a stray stderr write also lands on the alternate screen
    /// mid-frame. Pinned against a *real* `reqwest::Error`, not a synthesised string.
    #[test]
    fn a_failed_fetch_never_logs_the_signed_url_or_auth_token() {
        let client = reqwest::blocking::Client::builder()
            .connect_timeout(Duration::from_millis(500))
            .build()
            .expect("client builds");
        // Port 1 refuses instantly: no DNS, no network, no timeout.
        let url = SIGNED_STREAM_URL.replace("https://music.example.com", "http://127.0.0.1:1");
        let err = client
            .get(&url)
            .send()
            .expect_err("a connection to port 1 must fail");

        // The hazard is real: reqwest's own rendering does carry the token.
        assert!(
            err.to_string()
                .contains("t=5f4dcc3b5aa765d61d8327deb882cf99"),
            "reqwest stopped appending the URL — this test's premise needs rechecking: {err}"
        );

        let safe = safe_fetch_message(err);
        assert!(
            !safe.contains("t=5f4dcc3b5aa765d61d8327deb882cf99"),
            "leaked the auth token: {safe}"
        );
        assert!(!safe.contains("t="), "leaked a `t=` parameter: {safe}");
        assert!(!safe.contains("u="), "leaked a `u=` parameter: {safe}");
        assert!(!safe.contains("s="), "leaked an `s=` parameter: {safe}");
        assert!(!safe.contains("for url"), "leaked the URL clause: {safe}");
        assert!(!safe.contains("127.0.0.1:1"), "leaked the host: {safe}");
        let hex_run = safe
            .as_bytes()
            .windows(32)
            .any(|w| w.iter().all(|b| b.is_ascii_hexdigit()));
        assert!(!hex_run, "leaked a 32-char hex run: {safe}");

        // Non-vacuous: a message stripped to nothing would pass every assertion above.
        // `without_url` removes the URL, not the error — the diagnostic kind survives.
        assert!(
            safe.contains("error sending request"),
            "the message was stripped to nothing, so the assertions above prove nothing: \
             {safe:?}"
        );
    }
    ...
    fn eq_curve_contract() {
        ...
        let curve = eq_curve(vec![band], 0.0);
