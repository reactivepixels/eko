...
// the offline cache (`pro::offline`) and the parametric EQ (`pro::param_eq`) are
// all in `eko_core::pro::*`, reachable only under `--features pro`; what remains
// here are the `#[tauri::command]` wrappers in `commands::{license, offline}` and
// the `#[cfg(feature = "pro")]` seams below. The Pro dev examples moved with the
// licensing code to `crates/eko-core/examples/`.
...
///
/// `pub(crate)` because `commands::subsonic::subsonic_set_config` keeps it in step with
/// the configured server as well — see that command's docs. Only the proxy reads it.
#[derive(Default)]
pub(crate) struct StreamOrigin(pub(crate) Mutex<Option<String>>);
...
fn apply_pro_menu(app: &tauri::AppHandle) -> tauri::Result<()> {
    ...
    let config_dir = crate::commands::license::config_dir(app);
    let licensed = compute_status(config_dir.as_deref()).tier == Tier::Pro;
...
pub fn run() {
    ...
    let mut builder = tauri::Builder::default()
        .manage(eko_core::engine::Engine::default())
        .manage(StreamOrigin::default())
        // The OpenSubsonic client — empty until the frontend calls `subsonic_set_config`.
        // FREE: talking to your own server is not a Pro feature.
        .manage(commands::subsonic::SubsonicClient::default());
    ...
    builder
        ...
        // No `tauri_plugin_http`: it existed solely to give the old TypeScript Subsonic
        // client a CORS-free `fetch`, and that client is gone. Every remaining HTTP caller
        // is Rust-side and uses `reqwest` directly — `eko-net` for the OpenSubsonic API,
        // `stream.rs` for the audio/art proxy, and `tauri-plugin-updater` internally.
        // Its `http:default` capability entry went with it (a capability naming an
        // unregistered plugin fails the build, so the two must move together).
        ...
        .setup(|app| {
            ...
            // The AppHandle is needed to resolve the cache directory; `eko-core` is
            // Tauri-free, so we resolve it here and pass the path in. This expression
            // is verbatim what `OfflineCache::init` used to run against the handle —
            // changing it would silently relocate every existing user's offline library.
            // `init` still appends the `offline/` subdirectory itself.
            #[cfg(feature = "pro")]
            app.manage(eko_core::pro::offline::OfflineCache::init(
                h.path()
                    .app_cache_dir()
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            commands::metadata::scan_music_folder,
            commands::metadata::read_cover,
            commands::engine::engine_play,
            commands::engine::engine_play_url,
            commands::engine::engine_pause,
            commands::engine::engine_resume,
            commands::engine::engine_seek,
            commands::engine::engine_stop,
            commands::engine::engine_status,
            commands::engine::engine_bands,
            ...
            sync_visualizer,
            // ── OpenSubsonic (free — `eko-net`; credentials never reach the webview) ──
            commands::subsonic::subsonic_set_config,
            commands::subsonic::subsonic_ping,
            commands::subsonic::subsonic_get_albums,
            commands::subsonic::subsonic_get_songs,
            commands::subsonic::subsonic_get_album,
            commands::subsonic::subsonic_search,
            commands::subsonic::subsonic_get_playlists,
            commands::subsonic::subsonic_get_playlist,
            commands::subsonic::subsonic_get_random_songs,
            commands::subsonic::subsonic_get_genres,
            commands::subsonic::subsonic_get_album_list2,
            commands::subsonic::subsonic_get_similar_songs2,
            commands::subsonic::subsonic_get_artist_info2,
            commands::subsonic::subsonic_get_starred2,
            commands::subsonic::subsonic_scrobble,
            commands::subsonic::subsonic_get_lyrics_by_song_id,
            commands::subsonic::subsonic_get_lyrics_legacy,
            ...
            refresh_menu,
            #[cfg(feature = "pro")]
            commands::engine::engine_play_cached,
            #[cfg(feature = "pro")]
            commands::engine::engine_set_param_eq,
            #[cfg(feature = "pro")]
            commands::engine::engine_eq_curve,
            #[cfg(feature = "pro")]
            commands::engine::engine_parse_autoeq,
            #[cfg(feature = "pro")]
            commands::engine::engine_import_autoeq_file,
            #[cfg(feature = "pro")]
            commands::offline::cache_track,
            #[cfg(feature = "pro")]
            commands::offline::cache_album,
            #[cfg(feature = "pro")]
            commands::offline::remove_offline,
            #[cfg(feature = "pro")]
            commands::offline::offline_list,
            #[cfg(feature = "pro")]
            commands::offline::offline_stats,
            #[cfg(feature = "pro")]
            commands::offline::set_cache_limit,
            #[cfg(feature = "pro")]
        ...
        .run(|_app, event| {
            if let tauri::RunEvent::Exit = event {
                #[cfg(target_os = "macos")]
                eko_core::coreaudio::restore_all();
            }
        });
}
